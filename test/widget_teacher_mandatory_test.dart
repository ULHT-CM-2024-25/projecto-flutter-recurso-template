...
import 'fake_http_pharmacies_datasource.dart';
...
void runWidgetTests() {
  ...
  testWidgets('Has navigation bar with 4 options', (WidgetTester tester) async {
    await tester.pumpWidget(MultiProvider(
      providers: [
        ...
        Provider<HttpPharmaciesDataSource>.value(value: FakeHttpPharmaciesDataSource()),
      ],
      child: const MyApp(),
    ));
  ...
  testWidgets('Show hospitals and pharmacies map', (WidgetTester tester) async {
    await tester.pumpWidget(MultiProvider(
      providers: [
        Provider<HttpSnsDataSource>.value(value: FakeHttpSnsDataSource()),
        Provider<SqfliteSnsDataSource>.value(value: FakeSqfliteSnsDataSource()),
        Provider<LocationModule>.value(value: FakeLocationModule()),
        Provider<ConnectivityModule>.value(value: FakeConnectivityModule()),
        Provider<HttpPharmaciesDataSource>.value(value: FakeHttpPharmaciesDataSource(pharmacies: [
          Pharmacy(name: 'Farmácia 1', latitude: 38.73, longitude: -9.16, address: 'Rua'),
        ])),
      ],
      child: const MyApp(),
    ));
    ...
    final GoogleMap googleMap = tester.widget<GoogleMap>(mapFinder);
    // Ensure the map contains exactly two markers
    expect(googleMap.markers.length, 3, reason: "O mapa deve conter exatamente 3 marcadores");

    // Group markers by icon
    final Map<String, List<Marker>> groupedByIcon = {};

    for (final marker in googleMap.markers) {
      groupedByIcon.putIfAbsent(marker.icon.toJson().toString(), () => []).add(marker);
    }

    // There should be exactly 2 different icons (colors)
    expect(groupedByIcon.length, 2, reason: "Devem haver exatamente 2 cores diferentes de marcador pois há hospitais e farmácias");

    // One icon should have 2 markers, the other should have 1
    final markerCounts = groupedByIcon.values.map((group) => group.length).toList();
    markerCounts.sort(); // Sort so we can expect [1, 2]

    expect(markerCounts, [1, 2], reason: "Deve haver 2 marcadores de uma cor (hospitais) e 1 de outra (farmácias)");
  });
